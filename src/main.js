...
const pollingStationsUrl = new URL('../data/europawahllokale_2024.geojson', import.meta.url)
const constituenciesUrl = new URL('../data/europawahlbezirke_2024.geojson', import.meta.url)

const markerDefault = new URL('url:../static/marker-icon-default.webp', import.meta.url)
const markerSelected = new URL('url:../static/marker-icon-active.webp', import.meta.url)
...
fetch(constituenciesUrl, {
  method: 'GET'
}).then((response) => response.json()).then((data) => {
  addConstituenciesLayer(data)
}).catch(function (error) {
  console.log(error)
})
...
function addConstituenciesLayer(data) {
  L.geoJson(data, {
...
function renderFeatureDetails(feature) {
  ...
  const title = `${capitalizeEachWord(slug)} - Europawahl 2024 in Flensburg`
  ...
  detailOutput += `<li class="pb-2 text-xl lg:text-2xl"><strong>${label}</strong></li>`
  detailOutput += `<li class="last-of-type:pb-2 py-1 mb-3">${address}<br>${postal_code} ${city}</li>`
  detailOutput += '<li class="pt-2"><strong>Merkmal</strong><br>xxx</li>'
